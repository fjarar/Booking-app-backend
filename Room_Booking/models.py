...
from Room_Booking.supabase_storage import SupabaseStorage
...
class Room(models.Model):
    ...
    CURRENCY_TYPES = [("USD", "USD"), ("USD", "EUR")]
    ...
    maxOccupancy = models.IntegerField(default=1, validators=[MinValueValidator(1)])
...
class OccupiedDate(models.Model):
    room = models.ForeignKey(
        Room, on_delete=models.CASCADE, related_name="occupiedDates"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="booked_dates"
    )
